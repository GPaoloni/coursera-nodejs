...
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
  ...
  .then( (dish) => {
    if (dish !== null && dish.comments.id(req.params.commentId) !== null) {
      const author = dish.comments.id(req.params.commentId).author;
      if (req.user._id !== author) {
        const err = new Error('You can\'t modify someone\'s else comment!');
        err.status = 403;
        return next(err);
      }
      if (req.body.rating) {
...
.delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
  ...
  .then( (dish) => {
    if (dish !== null && dish.comments.id(req.params.commentId) !== null) {
      const author = dish.comments.id(req.params.commentId).author;
      if (req.user._id !== author) {
        const err = new Error('You can\'t delete someone\'s else comment!');
        err.status = 403;
        return next(err);
      }
